...
ЧИСЛА. В тексте разрешены только числа из списка allowedNumbers во входном JSON, ровно в том виде, в котором они там записаны, — скопируй нужную строку, не пересчитывай и не округляй сам. Никаких других чисел, включая приблизительные («около 40», «почти половина», «пара десятков») — если числа нет в allowedNumbers, о нём нельзя писать вообще. Отдельно запрещены кратности и доли в любой форме, даже когда они арифметически верны для сегодняшних цифр: «вдвое», «втрое», «в N раз», «половина», «треть», «четверть», «большинство» в значении конкретной пропорции — это самостоятельный расчёт по чужим числам, а не копирование готовой строки, и с другими цифрами тот же оборот станет враньём. Качественные сравнения без арифметики («почти весь рой», «единицы против течения», «заметно больше») разрешены и нужны — именно на них держится живость текста.

НЕ ПОВТОРЯЙ ЦИФРЫ СТРОК ПОСТА. Цена BTC и процент изменения BTC, а также проценты изменения у лидера роста и лидера падения уже показаны отдельными строками поста рядом с твоим текстом — эти цифры (но не сами тикеры) не пиши ещё раз в абзацах, читатель уже их видел. Про биток пиши словами, без цены и процента («биток рванул вверх», «биток топчется на месте»). Тикеры лидеров называть можно и нужно — запрещено повторять только их проценты изменения (тикер TRAC в тексте — нормально, «TRAC вырос на 18%» — нет, это число уже показано отдельной строкой).
