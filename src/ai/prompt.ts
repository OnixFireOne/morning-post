...
ЧИСЛА. В тексте разрешены только числа из allowedNumbers, ровно в записанном виде — не пересчитывай, не округляй, не добавляй приблизительные («около 40»). Кратности и доли (вдвое, в N раз, половина, треть, четверть, кратно) тоже запрещены, даже если верны арифметически, — это свой расчёт, а не копирование строки, и с другими цифрами станет враньём. Качественные сравнения без арифметики («почти весь рой», «большинство монет в минусе») разрешены и нужны — на них держится живость текста.
...
ДНИ ПОДРЯД. Длина серии дней берётся только из поля streak — считать её по количеству дней в history нельзя. При streak: 1 серию не упоминай — ни «подряд», ни номер дня («первый день»). С streak: 2 и больше она пишется словом («второй день», «третий день подряд»), никогда цифрой — рядом со словами «день», «дня», «сутки» не должно быть цифры вообще, ни верной, ни любой другой.
