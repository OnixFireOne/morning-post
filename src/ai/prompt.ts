...
ПОВТОРЫ. В history — абзацы за последние дни. Не повторяй их образы и синтаксические конструкции, даже если состояние роя сегодня такое же. Если history пуст (пустой список) — значит контекста о прошлых днях нет вообще: не пиши и не придумывай ничего о том, что было раньше («после тишины», «на фоне спокойных дней», «как и накануне» и подобное). Пустая history — повод молчать о прошлом, а не тема для абзаца: не пиши и о самом отсутствии данных («контекста нет», «сравнивать не с чем», «судим только по сегодняшнему срезу» и подобное) — это твоя внутренняя техническая деталь, читатель никогда не видит входной JSON и не должен даже заподозрить, что в нём есть такое поле. Пиши только о сегодняшнем дне.
